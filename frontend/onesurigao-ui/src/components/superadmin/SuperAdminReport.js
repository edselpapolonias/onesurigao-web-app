...
// ─── Report Detail Modal ──────────────────────────────────────────────────────
const LegacyReportDetailModal = ({ report, admins, superAdminID, onClose, onApprove, onDecline }) => {
    ...
    <div onClick={e => e.target === e.currentTarget && onClose()} style={MODAL_OVERLAY}>
      <div style={{ background: "linear-gradient(180deg,#FFFFFF 0%, #F8FBFF 100%)", border: "1px solid rgba(226,232,240,0.9)", borderRadius: 30, width: "100%", maxWidth: 700, maxHeight: "94vh", display: "flex", flexDirection: "column", boxShadow: "0 28px 70px rgba(15,23,42,0.18)", overflow: "hidden", animation: "slideUp 0.25s ease" }}>
        ...
        <div style={{ display: "flex", borderBottom: `1px solid ${DS.border}`, background: DS.card, flexShrink: 0 }}>
          ...
          <TabBtn id="validation" label="Validation" icon={<InfoIcon />} badge={report.status === "pending" ? 1 : 0} />
        </div>
        ...
        <div style={{ overflowY: "auto", flex: 1, padding: "22px 26px" }}>
          ...
          {activeTab === "validation" && (
            <>
              {report.status === "pending" && (
                <div style={{ background: DS.bg, borderRadius: 12, padding: "18px", border: `1.5px solid ${DS.border}` }}>
                  <div style={{ fontSize: 11, fontWeight: 700, color: DS.textMuted, marginBottom: 14, fontFamily: DS.font, textTransform: "uppercase", letterSpacing: 0.6 }}>Validate this Report</div>
                  <div style={{ display: "flex", gap: 10, marginBottom: 14 }}>
                    <button onClick={() => setMode(mode === "approve" ? null : "approve")} style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", gap: 6, background: mode === "approve" ? "linear-gradient(135deg,#276749,#38A169)" : "#F0FDF4", border: `1.5px solid ${mode === "approve" ? "#38A169" : "#9AE6B4"}`, borderRadius: 8, padding: "11px", cursor: "pointer", fontSize: 13, fontWeight: 700, color: mode === "approve" ? "#fff" : "#276749", fontFamily: DS.font }}>
                      <CheckIcon /> Approve
                    </button>
                    <button onClick={() => setMode(mode === "decline" ? null : "decline")} style={{ flex: 1, display: "flex", alignItems: "center", justifyContent: "center", gap: 6, background: mode === "decline" ? "linear-gradient(135deg,#9B2C2C,#C53030)" : "#FFF5F5", border: `1.5px solid ${mode === "decline" ? "#C53030" : "#FEB2B2"}`, borderRadius: 8, padding: "11px", cursor: "pointer", fontSize: 13, fontWeight: 700, color: mode === "decline" ? "#fff" : "#C53030", fontFamily: DS.font }}>
                      <XIcon /> Decline
                    </button>
                  </div>

                  {mode === "approve" && (
                    <div style={{ background: "#F0FDF4", border: "1.5px solid #9AE6B4", borderRadius: 10, padding: "16px" }}>
                      <label style={{ display: "block", marginBottom: 8, fontWeight: 600, fontSize: 11, color: "#276749", fontFamily: DS.font, textTransform: "uppercase", letterSpacing: 0.6 }}>Assign to Office <span style={{ color: "#DC2626" }}>*</span></label>
                      <select value={assignedTo} onChange={e => setAssignedTo(e.target.value)} style={{ ...inputSt, marginBottom: 12, cursor: "pointer", borderColor: "#9AE6B4" }}>
                        <option value="">Select Office</option>
                        {admins.map(a => <option key={a.adminID} value={a.adminID}>{a.officeName}</option>)}
                      </select>
                      <button onClick={handleApprove} disabled={submitting || !assignedTo} style={{ width: "100%", padding: "11px", fontSize: 13, fontWeight: 700, background: submitting || !assignedTo ? "#9AB8E0" : "linear-gradient(135deg,#276749,#38A169)", color: "#fff", border: "none", borderRadius: 8, cursor: submitting || !assignedTo ? "not-allowed" : "pointer", fontFamily: DS.font, display: "flex", alignItems: "center", justifyContent: "center", gap: 6 }}>
                        <CheckIcon />{submitting ? "Approving..." : "Confirm Approval"}
                      </button>
                    </div>
                  )}
                  ...
                  {mode === "decline" && (
                    <div style={{ background: "#FFF5F5", border: "1.5px solid #FEB2B2", borderRadius: 10, padding: "16px" }}>
                      <label style={{ display: "block", marginBottom: 8, fontWeight: 600, fontSize: 11, color: "#C53030", fontFamily: DS.font, textTransform: "uppercase", letterSpacing: 0.6 }}>Reason for Declining <span style={{ color: "#DC2626" }}>*</span></label>
                      <textarea value={declineReason} onChange={e => setDeclineReason(e.target.value)} rows={3} placeholder="Explain why this report is being declined..." style={{ ...inputSt, resize: "vertical", marginBottom: 12, borderColor: "#FEB2B2" }} onFocus={e => e.target.style.borderColor = "#C53030"} onBlur={e => e.target.style.borderColor = "#FEB2B2"} />
              ...
              {report.status !== "pending" && (
                <div style={{ display: "grid", gap: 14 }}>
                  ...
                  {report.adminResponses?.length > 0 && (
                    <div>
                      <div style={{ fontSize: 11, fontWeight: 700, color: DS.textMuted, marginBottom: 12, fontFamily: DS.font, textTransform: "uppercase", letterSpacing: 0.6 }}>Office Responses ({report.adminResponses.length})</div>
