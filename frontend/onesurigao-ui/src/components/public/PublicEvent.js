...
const TabButton = ({ label, active, onClick, count }) => (
  <button onClick={onClick} style={{ display: "flex", alignItems: "center", gap: 7, padding: "8px 18px", borderRadius: 8, border: "none", cursor: "pointer", fontSize: 13, fontWeight: active ? 700 : 500, background: active ? "linear-gradient(135deg, #1a56a0, #1976d2)" : "#fff", color: active ? "#fff" : "#555", fontFamily: "'Segoe UI', sans-serif", boxShadow: active ? "0 2px 8px rgba(25,118,210,0.3)" : "0 1px 4px rgba(0,0,0,0.07)", transition: "all 0.2s" }}>
    {label}
    {count !== undefined && <span style={{ background: active ? "rgba(255,255,255,0.25)" : "#e0e7ef", color: active ? "#fff" : "#555", borderRadius: 12, padding: "1px 7px", fontSize: 11, fontWeight: 700 }}>{count}</span>}
  </button>
);
...
const EventCard = ({ event, onClick, isPast = false }) => {
  ...
  return (
    <div onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)}
      style={{ background: "#fff", borderRadius: 10, overflow: "hidden", boxShadow: hovered ? "0 6px 24px rgba(0,0,0,0.13)" : "0 2px 10px rgba(0,0,0,0.08)", transition: "box-shadow 0.2s, transform 0.2s", transform: hovered ? "translateY(-2px)" : "translateY(0)", opacity: isPast ? 0.85 : 1 }}
    >
      <div style={{ height: 120, background: isPast ? "linear-gradient(135deg, #555, #333)" : "linear-gradient(135deg, #1a56a0, #0d3b7a)", overflow: "hidden", position: "relative" }}>
        {posterUrl ? (
          <img src={posterUrl} alt={event.title} style={{ width: "100%", height: "100%", objectFit: "cover", transition: "transform 0.3s", transform: hovered ? "scale(1.04)" : "scale(1)", filter: isPast ? "grayscale(40%)" : "none" }} />
        ) : (
...
function PublicEvent() {
  ...
  // Split into upcoming and past based on eventDate
    ...
    <Layout>
      ...
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 20, flexWrap: "wrap", gap: 10 }}>
        <div>
          ...
          <p style={{ margin: "4px 0 0", fontSize: 13, color: "#888", fontFamily: "'Segoe UI', sans-serif" }}>
            {activeTab === "UPCOMING" ? "Community Gatherings, Holidays, Official Schedules" : "Events that have already taken place"}
          </p>
        </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 14 }}>
        ...
        {!loading && displayEvents.map(event => (
          <EventCard key={event.eventID || event.id} event={event} onClick={setSelectedEvent} isPast={activeTab === "PAST"} />
        ))}
        ...
        {!loading && !error && displayEvents.length === 0 && (
          <div style={{ gridColumn: "1 / -1", background: "#fff", borderRadius: 10, padding: "48px 20px", textAlign: "center", color: "#aaa", fontSize: 14, fontFamily: "'Segoe UI', sans-serif", boxShadow: "0 2px 10px rgba(0,0,0,0.07)" }}>
            <div style={{ fontSize: 36, marginBottom: 12 }}>{activeTab === "UPCOMING" ? "🗓️" : "📅"}</div>
