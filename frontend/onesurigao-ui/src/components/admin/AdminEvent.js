...
const AddEventModal = ({ onClose, onSubmit, adminOfficeName }) => {
  const [form, setForm] = useState({
    title: "", description: "", eventDate: "", location: "", posterFile: null,
  });
  ...
  const handleSubmit = async () => {
    if (!form.title || !form.eventDate || !form.location) {
      alert("Please fill in Title, Event Date, and Location.");
      return;
    }
    setSubmitting(true);
    try {
  ...
  const labelStyle = {
    ...
    fontSize: 12, color: "#555", fontFamily: "'Segoe UI', sans-serif",
      ...
      <div style={{ background: "#fff", borderRadius: 14, width: "100%", maxWidth: 680, maxHeight: "92vh", display: "flex", flexDirection: "column", boxShadow: "0 20px 60px rgba(0,0,0,0.3)", overflow: "hidden" }}>
        ...
        <div style={{ background: "linear-gradient(135deg, #1a56a0, #1976d2)", padding: "20px 28px", display: "flex", alignItems: "center", justifyContent: "space-between", flexShrink: 0 }}>
          <div>
            ...
            <div style={{ color: "rgba(255,255,255,0.7)", fontSize: 12, fontFamily: "'Segoe UI', sans-serif", marginTop: 2 }}>
              Submit for Super Admin approval before publishing
            </div>
          </div>
        ...
        <div style={{ overflowY: "auto", flex: 1, padding: "24px 28px" }}>
          ...
          <div style={{ marginBottom: 16 }}>
            ...
            <input name="title" value={form.title} onChange={handleChange} placeholder="e.g. Bonok-Bonok Festival" style={inputStyle}
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14, marginBottom: 16 }}>
            <div>
              ...
              <input type="datetime-local" name="eventDate" value={form.eventDate} onChange={handleChange} style={inputStyle}
                onFocus={e => e.target.style.borderColor = "#1976d2"} onBlur={e => e.target.style.borderColor = "#dde3ec"} />
            </div>
            <div>
              <label style={labelStyle}>Location <span style={{ color: "#e53935" }}>*</span></label>
              <input name="location" value={form.location} onChange={handleChange} placeholder="e.g. Surigao Provincial Sports..." style={inputStyle}
                onFocus={e => e.target.style.borderColor = "#1976d2"} onBlur={e => e.target.style.borderColor = "#dde3ec"} />
            </div>
          </div>
          ...
          <div style={{ marginBottom: 16 }}>
            ...
            <textarea name="description" value={form.description} onChange={handleChange} rows={4}
              placeholder="Describe the event, activities, what to expect..."
          ...
          <div style={{ marginBottom: 8 }}>
            <label style={labelStyle}>Event Poster / Cover Photo <span style={{ fontSize: 11, color: "#aaa", fontWeight: 400, textTransform: "none", letterSpacing: 0 }}>(one image, shown on card)</span></label>

            {preview ? (
              <div style={{ position: "relative", borderRadius: 10, overflow: "hidden", border: "1.5px solid #dde3ec" }}>
                <img src={preview} alt="poster preview" style={{ width: "100%", height: 200, objectFit: "cover", display: "block" }} />
                <button
                  onClick={() => { setPreview(null); setForm(f => ({ ...f, posterFile: null })); }}
                ...
                ><XIcon /></button>
                <div style={{ position: "absolute", bottom: 10, left: 10, background: "rgba(0,0,0,0.55)", color: "#fff", fontSize: 11, borderRadius: 6, padding: "3px 10px", fontFamily: "'Segoe UI', sans-serif" }}>
              ...
              <div
                ...
                style={{ border: `2px dashed ${dragOver ? "#1976d2" : "#c8d6e8"}`, borderRadius: 10, padding: "32px 16px", textAlign: "center", cursor: "pointer", background: dragOver ? "#eef5ff" : "#f8fafd", transition: "all 0.2s" }}
              >
                ...
                <p style={{ margin: 0, fontSize: 13, color: "#666", fontFamily: "'Segoe UI', sans-serif" }}>
                  <span style={{ color: "#1976d2", fontWeight: 700 }}>Click to upload</span> or drag & drop
                </p>
                <p style={{ margin: "4px 0 0", fontSize: 11, color: "#aaa", fontFamily: "'Segoe UI', sans-serif" }}>PNG, JPG, GIF — one image for the event card</p>
              </div>
            )}
          </div>
        </div>
        ...
        <div style={{ padding: "16px 28px", borderTop: "1px solid #f0f0f0", display: "flex", justifyContent: "flex-end", gap: 10, flexShrink: 0, background: "#fafbfc" }}>
          ...
          <button
            ...
            disabled={submitting}
            style={{ background: submitting ? "#9ab8e0" : "linear-gradient(135deg, #1a56a0, #1976d2)", color: "#fff", border: "none", padding: "10px 26px", fontSize: 13, fontWeight: 700, borderRadius: 8, cursor: submitting ? "not-allowed" : "pointer", fontFamily: "'Segoe UI', sans-serif", boxShadow: "0 2px 8px rgba(25,118,210,0.3)", display: "flex", alignItems: "center", gap: 7 }}
          >
    ...
    <div onClick={(e) => e.target === e.currentTarget && onClose()} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.55)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000, padding: 20 }}>
      <div style={{ background: "#fff", borderRadius: 14, width: "100%", maxWidth: 560, maxHeight: "88vh", display: "flex", flexDirection: "column", boxShadow: "0 20px 60px rgba(0,0,0,0.3)", overflow: "hidden" }}>
        ...
        <div style={{ overflowY: "auto", flex: 1, padding: "24px" }}>
          <h2 style={{ margin: "0 0 16px", fontSize: 20, fontWeight: 800, color: "#1a1a1a", fontFamily: "'Segoe UI', sans-serif", textTransform: "uppercase", letterSpacing: 0.5 }}>{event.title}</h2>
          ...
          {event.description && (
            <div>
              <div style={{ fontSize: 12, fontWeight: 700, color: "#888", textTransform: "uppercase", letterSpacing: 0.5, marginBottom: 8, fontFamily: "'Segoe UI', sans-serif" }}>About this Event</div>
              <p style={{ fontSize: 14, color: "#444", lineHeight: 1.75, fontFamily: "'Segoe UI', sans-serif", margin: 0 }}>{event.description}</p>
            </div>
          )}
        </div>
      </div>
...
const EventCard = ({ event, onClick }) => {
  ...
  return (
    <div
      ...
      style={{ background: "#fff", borderRadius: 10, overflow: "hidden", boxShadow: hovered ? "0 6px 24px rgba(0,0,0,0.13)" : "0 2px 10px rgba(0,0,0,0.08)", transition: "box-shadow 0.2s, transform 0.2s", transform: hovered ? "translateY(-2px)" : "translateY(0)", cursor: "pointer" }}
    >
      {/* Poster */}
      <div style={{ height: 120, background: "linear-gradient(135deg, #1a56a0, #0d3b7a)", overflow: "hidden", position: "relative" }}>
        {event.posterUrl ? (
          <img src={event.posterUrl} alt={event.title} style={{ width: "100%", height: "100%", objectFit: "cover", transition: "transform 0.3s", transform: hovered ? "scale(1.04)" : "scale(1)" }} />
        ) : (
        ...
        {/* Pending badge */}
        {event.isPending && (
          <div style={{ position: "absolute", top: 8, right: 8, background: "#f59e0b", color: "#fff", fontSize: 10, fontWeight: 700, padding: "3px 8px", borderRadius: 20, fontFamily: "'Segoe UI', sans-serif" }}>
            PENDING
          </div>
        )}
      </div>
  ...
  return (
    ...
    >
      <div style={{
        ...
        background: hovered ? "linear-gradient(135deg, #1a56a0, #1976d2)" : "#1976d2",
        display: "flex", alignItems: "center", justifyContent: "center",
        marginBottom: 16, transition: "all 0.2s",
        boxShadow: hovered ? "0 6px 20px rgba(25,118,210,0.4)" : "0 3px 12px rgba(25,118,210,0.3)",
        color: "#fff",
      }}>
        <PlusIcon />
      </div>
      <span style={{ fontSize: 14, fontWeight: 800, color: "#1976d2", fontFamily: "'Segoe UI', sans-serif", letterSpacing: 1, textTransform: "uppercase" }}>
        ADD EVENT
      </span>
    </div>
  );
...
function AdminEvent() {
  ...
  useEffect(() => {
    setLoading(true);
    // ✅ Pass adminID so the backend returns this admin's pending events too
    ...
    axios.get(API_URL, { params })
      ...
      .catch(() => setError("Failed to load events. Is your Django server running?"))
  ...
  const handleSubmitEvent = async (form) => {
    ...
    formData.append("admin_id", adminID); // ✅ matches serializer field name
    ...
    // ✅ Add to local list immediately — isPending shown until refresh confirms status
    const newEvent = {
      ...res.data,
      posterUrl: form.posterFile ? URL.createObjectURL(form.posterFile) : null,
      postedBy: officeName,
      isPending: !res.data.isApproved,
    };
    setEvents(prev => [newEvent, ...prev]);
  };

  // Enrich events with posterUrl and pending status for display
  const enrichedEvents = events.map(e => ({
    ...e,
    // ✅ use_url=True means posterPath is already a full URL — use it directly
    posterUrl: e.posterUrl || e.posterPath || null,
    postedBy: e.admin?.officeName || officeName || "Surigao PIO",
    isPending: !e.isApproved,
  }));
  ...
  return (
    <Layout>
      ...
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 20, flexWrap: "wrap", gap: 10 }}>
        <div>
          ...
          <p style={{ margin: "4px 0 0", fontSize: 13, color: "#888", fontFamily: "'Segoe UI', sans-serif" }}>
            Community Gathering, Holidays, Official Schedules
          </p>
          {officeName && (
            <p style={{ margin: "4px 0 0", fontSize: 12, color: "#1976d2", fontFamily: "'Segoe UI', sans-serif", fontWeight: 600 }}>
        ...
        <button
          onClick={() => {}}
          style={{ display: "flex", alignItems: "center", gap: 8, background: "linear-gradient(135deg, #1a56a0, #1976d2)", border: "none", padding: "10px 20px", borderRadius: 8, cursor: "pointer", fontSize: 13, fontWeight: 700, color: "#fff", fontFamily: "'Segoe UI', sans-serif", boxShadow: "0 2px 8px rgba(25,118,210,0.3)", letterSpacing: 0.5 }}
        >
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 14 }}>
        ...
        {/* Event cards */}
        {!loading && enrichedEvents.map((event) => (
          <EventCard
        ...
        {/* Empty state (besides the Add card) */}
        {!loading && !error && enrichedEvents.length === 0 && (
          <div style={{ background: "#fff", borderRadius: 10, padding: "48px 20px", textAlign: "center", color: "#aaa", fontSize: 14, fontFamily: "'Segoe UI', sans-serif", boxShadow: "0 2px 10px rgba(0,0,0,0.07)", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center" }}>
            <div style={{ fontSize: 36, marginBottom: 12 }}>🗓️</div>
            No events yet. Click <strong style={{ margin: "0 4px" }}>ADD EVENT</strong> to create one.
          </div>
        )}
      </div>
