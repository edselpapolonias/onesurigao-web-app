...
const ResolveModal = ({ report, onClose, onConfirm }) => {
  ...
  return (
    <div onClick={e=>e.target===e.currentTarget&&onClose()} style={MODAL_OVERLAY}>
      <div style={{background:"linear-gradient(180deg,#FFFFFF 0%, #F8FBFF 100%)",border:"1px solid rgba(226,232,240,0.9)",borderRadius:30,width:"100%",maxWidth:460,boxShadow:"0 28px 70px rgba(15,23,42,0.18)",overflow:"hidden",animation:"slideUp 0.25s ease"}}>
        <div style={{padding:"18px 24px",display:"flex",justifyContent:"space-between",alignItems:"center",borderBottom:`1px solid ${DS.border}`,background:"linear-gradient(180deg,#FFFFFF 0%, #F7FBFF 100%)"}}>
          <div style={{color:DS.textPrimary,fontWeight:800,fontSize:15,fontFamily:DS.font}}>Mark as Resolved</div>
...
const ReportDetailModal = ({ report, onClose, onRespond, onResolve, onRefresh }) => {
  ...
  return (
    <>
      <div onClick={e=>e.target===e.currentTarget&&onClose()} style={{...MODAL_OVERLAY, zIndex:1000}}>
