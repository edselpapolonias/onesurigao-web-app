...
const AnnouncementForm = ({ initial = { title: "", content: "" }, onPost, onSaveDraft, onCancel }) => {
  const [form, setForm] = useState(initial);
  useEffect(() => setForm(initial), [initial.title, initial.content]);
  ...
  return (
    <div style={{ padding: "24px" }}>
      <div style={{ marginBottom: 16 }}>
        <label style={{ display: "block", marginBottom: 6, fontWeight: 600, fontSize: 13, color: "#444", fontFamily: "'Segoe UI', sans-serif" }}>Title</label>
        <input type="text" name="title" value={form.title} onChange={handleChange} placeholder="Enter announcement title..." style={inputStyle}
          onFocus={(e) => e.target.style.borderColor = "#1976d2"} onBlur={(e) => e.target.style.borderColor = "#dde3ec"} />
      </div>
      <div style={{ marginBottom: 24 }}>
        <label style={{ display: "block", marginBottom: 6, fontWeight: 600, fontSize: 13, color: "#444", fontFamily: "'Segoe UI', sans-serif" }}>Content</label>
        <textarea name="content" value={form.content} onChange={handleChange} rows={5} placeholder="Write the announcement content here..."
          style={{ ...inputStyle, resize: "vertical" }}
          onFocus={(e) => e.target.style.borderColor = "#1976d2"} onBlur={(e) => e.target.style.borderColor = "#dde3ec"} />
      </div>
      <div style={{ display: "flex", gap: 10, justifyContent: "flex-end" }}>
        <button onClick={onCancel} style={{ background: "#f0f2f5", border: "none", padding: "10px 20px", borderRadius: 8, cursor: "pointer", fontSize: 13, fontWeight: 600, color: "#555", fontFamily: "'Segoe UI', sans-serif" }}>Cancel</button>
        {onSaveDraft && (
          <button onClick={() => form.title && onSaveDraft(form)} style={{ background: "#fff", border: "1.5px solid #1976d2", padding: "10px 20px", borderRadius: 8, cursor: "pointer", fontSize: 13, fontWeight: 600, color: "#1976d2", fontFamily: "'Segoe UI', sans-serif" }}>Save as Draft</button>
        )}
        <button onClick={() => form.title && form.content && onPost(form)} style={{ background: "linear-gradient(135deg, #1a56a0, #1976d2)", color: "#fff", border: "none", padding: "10px 22px", fontSize: 13, fontWeight: 700, borderRadius: 8, cursor: "pointer", fontFamily: "'Segoe UI', sans-serif", boxShadow: "0 2px 8px rgba(25,118,210,0.25)" }}>Post Announcement</button>
      </div>
    </div>
  );
};
...
const PostModal = ({ onClose, onPost, onSaveDraft, editDraft = null }) => (
  <div onClick={(e) => e.target === e.currentTarget && onClose()} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.45)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000 }}>
    <div style={{ background: "#fff", borderRadius: 12, width: "100%", maxWidth: 580, boxShadow: "0 8px 40px rgba(0,0,0,0.2)", overflow: "hidden" }}>
      <div style={{ background: "linear-gradient(135deg, #1a56a0, #1976d2)", padding: "16px 24px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <span style={{ color: "#fff", fontWeight: 700, fontSize: 16, fontFamily: "'Segoe UI', sans-serif" }}>{editDraft ? "Edit Draft" : "Post Announcement"}</span>
      ...
      <AnnouncementForm initial={editDraft || { title: "", content: "" }} onPost={onPost} onSaveDraft={onSaveDraft} onCancel={onClose} />
    </div>
  </div>
);
...
const AnnouncementCard = ({ announcement, currentAdminID, onPin }) => {
  ...
  // ✅ Only the admin who posted can pin/unpin
              ...
              {isOwner ? (
                <button
                  ...
                  style={{
                    ...
                    color: isPinned ? "#d97706" : "#333",
                    fontFamily: "'Segoe UI', sans-serif", textAlign: "left",
                  }}
                  onMouseEnter={(e) => e.currentTarget.style.background = "#f9fafb"}
                  onMouseLeave={(e) => e.currentTarget.style.background = "none"}
                >
                  <span style={{ color: isPinned ? "#d97706" : "#888", display: "flex" }}>
...
function AdminAnnouncement() {
  ...
  // ✅ Toggle pin — PATCH isPinned to Django
